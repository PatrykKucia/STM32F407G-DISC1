...
void main_test(void)
{
	drawtestpage((uint8_t *)"Œ¢—©µÁ◊”◊€∫œ≤‚ ‘≥Ã–Ú");	
	gui_strcenter(0,30,RED,BLUE,(uint8_t *)"Œ¢—©µÁ◊”",16,1);
...
{
	uint8_t i=0;
	drawtestpage((uint8_t *)"≤‚ ‘3:GUIæÿ–ŒÃÓ≥‰≤‚ ‘");
...
{
	uint8_t i=0;
	drawtestpage((uint8_t *)"≤‚ ‘4:GUIª≠‘≤ÃÓ≥‰≤‚ ‘");
	lcd_fill(0,20,lcddev.width-1,lcddev.height-20,WHITE);
	for (i=0; i<5; i++)  
...
{
	uint8_t i=0;
	drawtestpage((uint8_t *)"≤‚ ‘5:GUI TriangleÃÓ≥‰≤‚ ‘");
	lcd_fill(0,20,lcddev.width-1,lcddev.height-20,WHITE);
	for(i=0;i<5;i++)
	{
		POINT_COLOR=ColorTab[i];
...
{	
	drawtestpage((uint8_t *)"≤‚ ‘7:÷–Œƒœ‘ æ≤‚ ‘");
	show_str(10,30,BLUE,YELLOW,(uint8_t *)"16X16:Œ¢—©µÁ◊””–œﬁπ´Àæª∂”≠ƒ˙",16,0);
	show_str(10,50,BLUE,YELLOW,(uint8_t *)"16X16:WelcomeŒ¢—©µÁ◊”",16,0);
	show_str(10,70,BLUE,YELLOW,(uint8_t *)"24X24:…Ó€⁄ –÷–Œƒ≤‚ ‘",24,1);
...
void touch_test(void)
{
	...
	lcd_showstring(lcddev.width-24,0,16,(uint8_t *)"RST",1);//œ‘ æ«Â∆¡«¯”Ú
	POINT_COLOR=RED;
	...
	while(1)
	{
		...
		{	
			...
			{	
				...
				{
					drawtestpage((uint8_t *)"≤‚ ‘10:Touch≤‚ ‘   ");
					lcd_showstring(lcddev.width-24,0,16,(uint8_t *)"RST",1);
